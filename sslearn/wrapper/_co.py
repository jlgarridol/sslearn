...
class DemocraticCoLearning(_BaseCoTraining):
    ...
    def __calc_y_ponderated(self, predictions, confidences=None):
        y_complete = None
        if confidences is None:
            confidences = self.confidences_
        for w, p in zip(confidences, predictions):
            y_ = self.one_hot.transform(p)*w
            if y_complete is None:
                y_complete = y_
            else:
                y_complete += y_
        
        y_hot = np.zeros(y_complete.shape)
        y_hot[:, np.argmax(y_complete, axis=1)] = 1

        return self.one_hot.inverse_transform(y_hot)

    def __calc_predictions(self, X, estimators=None):
    ...
    def fit(self, X, y, estimator_kwards=None):
        ...
        while changed:
            ...
            predictions = list(self.__calc_predictions(X_unlabel, estimators=self.base_estimator))
            y_predicted = self.__calc_y_ponderated(predictions, confidences=confidence)
            ...
            for i in range(self.n_estimators):
                to_add = predictions[i] == y_predicted
                L_[i] = np.concatenate(
                    (L_[i], X_unlabel[to_add,:]), axis=0)
                Ly_[i] = np.concatenate(
                    (L_[i], y_predicted[to_add]), axis=0)
